<?php
	...
	if ($result->num_rows) {
		...
		while($row = $result->fetch_assoc()) {
			$item .= "
			<tr>
				<td><a href='update_album.php?id_album=". $row["id_album"] ."'>". $row["title"] ."</a></td>
				<td>". $row["date"] ."</td>
				<td>". $row["country"] ."</td>
			</tr>
			";
			...
			<div class="content">
				<div class="center">
					<form action="crud.php" method="POST">
						<h2>Изменить данные группы <?php echo "'". $team["name"] ."'" ?></h2><br>
						<input type="text" placeholder="Название" name="name" value="<?php echo $team["name"] ?>">
						<input type="text" placeholder="Страна" name="country" value="<?php echo $team["country"] ?>">
						<input type="text" placeholder="Дата" name="date" value="<?php echo $team["date"] ?>">
						<input type="text" placeholder="Стиль" name="style" value="<?php echo $team["style"] ?>">
