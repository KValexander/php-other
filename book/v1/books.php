	...
	while($row = $result->fetch_assoc()) {
		...
		$data .= sprintf("
			<tr>
				...
				<td>%s</td>
				<td>%s</td>
				<td><a href='add_book.php?book_id=%s'>Изменить</a></td>
				<td><a href='delete.php?book_id=%s'>Удалить</a></td>
			</tr>
		", $row["name"], $row["year_of_creation"], $row["description"], $row["book_id"], $row["book_id"]);
	}
	if($data == "") $data = "<tr><td colspan=5>Данные отсутствуют</td></tr>";
